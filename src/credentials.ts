...
/**
 * The DeepSeek key, read only when the provider is 'deepseek' (src/llm.ts).
 *
 * Its own name rather than a shared "llm_api_key": two vendors, two balances, two 401s. A single
 * slot would also make switching providers destroy the key for the one being left.
 */
 ...
 * typed. The chosen exit vendor is Proxy-Cheap static residential, US Pacific
 * (PROXY-VENDOR-DECISION.md) — not Webshare, whose paid ISP tier carries a 20-IP minimum. This
...
export async function anthropicKeyFromVault(operator?: string | null): Promise<string | null> {
  return providerKeyFromVault(ANTHROPIC_API_KEY, operator);
}

/**
 * The DeepSeek key, from the vault, for this operator or the machine.
 *
 * Identical resolution to the Anthropic one — operator scope first, then global — because the
 * reason is identical: two operators on one machine bill different accounts.
 */
export async function deepseekKeyFromVault(operator?: string | null): Promise<string | null> {
  return providerKeyFromVault(DEEPSEEK_API_KEY, operator);
}
 ...
 * what to say about a missing key, because it also knows about the environment variable.
 */
async function providerKeyFromVault(name: string, operator?: string | null): Promise<string | null> {
  if (!vaultReady()) return null;
  if (operator && NAME_RE.test(operator)) {
    const mine = await getSecret(name, operator);
    if (mine) return mine;
  }
  return getSecret(name, SCOPE_GLOBAL);
}
