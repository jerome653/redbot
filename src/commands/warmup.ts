 ...
 */
function minutesSinceLastPublish(account: string): number | null {
...
export async function warmup(opts?: { dry?: boolean }): Promise<number> {
  ...
  const c = counters(account.handle, new Date(), undefined, account.timezone);
  const pace = checkWarmingPace({
    publishedToday: c.repliesToday,
    minutesSinceLast: minutesSinceLastPublish(account.handle),
    karma: c.karma
  });
  ...
  if (!candidates.length) {
    ...
    say.warn('warming needs threads under 2h, and a batch collected hours ago has none left.');
    return 1;
  }
