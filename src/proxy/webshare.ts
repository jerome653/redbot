 ...
 * operator still has to run through the same six-hour vet as a hand-typed one. The chosen exit
 * vendor is Proxy-Cheap static residential, US Pacific (PROXY-VENDOR-DECISION.md). A Webshare key
 * is a shortcut for whoever already has one, and its cheap pool is datacenter-classified, which
 * the vet is there to catch.
 *
